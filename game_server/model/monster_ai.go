...
import (
	...
	"github.com/NumberMan1/MMO-server/game_server/core/fsm"
...
func NewWalkState() *WalkState {
	return &WalkState{
		...
		lastTime: game_common.GetScheduleInstance().Clock().Time,
		waitTime: 10,
	}
}
...
func (ws *WalkState) OnUpdate() {
	...
	if mon.State() == proto.EntityState_IDLE {
		if ws.lastTime+ws.waitTime < game_common.GetScheduleInstance().Clock().Time {
			ws.lastTime = game_common.GetScheduleInstance().Clock().Time
