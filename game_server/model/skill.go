...
func (s *Skill) Update() {
	...
	if s.Cd > 0 {
		s.Cd -= game_common.GetScheduleInstance().Clock().DeltaTime
		return
	}
	if s.Cd < 0 {
		s.Cd = 0
	...
	s.time += game_common.GetScheduleInstance().Clock().DeltaTime
...
func (s *Skill) OnActive() {
	...
	if s.Def.IsMissile {
		...
		for _, v := range s.Def.HitDelay {
			game_common.GetScheduleInstance().AddTask(s.hitTrigger, time.Millisecond*time.Duration(v*1000), 1)
		}
	}
}

// 触发延迟伤害
func (s *Skill) hitTrigger() error {
	logger.SLCInfo("hitTrigger：owner[%v],Skill[%v]", s.Owner.EntityId(), s.Def.Name)
	return s.OnHit(s.target)
}

// OnHit 技能打到目标
func (s *Skill) OnHit(sco fight.ISCObject) error {
