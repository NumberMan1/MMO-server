...
import (
	...
	"github.com/NumberMan1/MMO-server/game_server/bootstrap"
	...
	"github.com/NumberMan1/MMO-server/game_server/service"
...
func initServices() {
	...
	game_common.GetScheduleInstance().Start(context.Background())
	...
	game_common.GetScheduleInstance().Update(func() error {
		mgr.GetEntityManagerInstance().Update()
		model.GetSpaceManagerInstance().Update()
		return nil
	})
...
func main() {
	bootstrap.RootCmd.Run = func(cmd *cobra.Command, args []string) {
		initServices()
		select {}
	}
	// Execute the root command
	if err := bootstrap.RootCmd.Execute(); err != nil {
		fmt.Println(err)
		os.Exit(1)
	}
}
